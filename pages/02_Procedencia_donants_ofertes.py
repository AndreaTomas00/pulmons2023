...
st.write("# Procedència ofertes pulmonars")
...
color_procedencia = alt.Scale(domain=[ 'Pool', 'Propi',  'Balears', 'ONT', 'FOEDUS'],
                        range=['#FF8000', '#FFD966',   '#BBD2EC', '#3E77B6', '#badd73'], 
                        )
orden = ["Propi", "Pool", "ONT", "Balears", "FOEDUS"]
sortt = ["OCATT", "ONT", "FOEDUS"]
# Create the bar chart
chart_propis = alt.Chart(data).mark_bar().encode(
    x=alt.X('ORGANITZACIÓ:N', title='Organització de procedència', axis=alt.Axis(labelAngle=0), scale=alt.Scale(domain=["OCATT", "ONT", "FOEDUS"])),
